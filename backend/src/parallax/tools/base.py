...
class ToolContext:
    ...
    Empty of real dependencies today because every tool returns canned data.
    It exists so that adding a database or an HTTP client later is a change to
    this one class, not to every tool signature.
    """
...
@dataclass
class ToolRegistry:
    """A named set of tools. One registry per agent, so agents cannot reach
    into each other's capabilities by accident."""
    ...
    async def call(self, name: str, raw_arguments: str, ctx: ToolContext) -> str:
        ...
        A bad tool name or malformed arguments is the model's mistake, and it
        can recover if we hand the error back as an observation. Letting it
        propagate would abort a run the model could have fixed itself.
        """
